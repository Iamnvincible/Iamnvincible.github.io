...
除了为准备环境引入的运行库，要使用 C 语言功能还需要 C 语言标准库。标准库提供了最基本的能使用文件、I/O、字符串、堆等功能的函数，这些函数声明在常见的头文件中，如 `stdio.h`、`string.h`、`stdlib.h` 等。GLIBC 就是最为常见的 C 语言标准库，除了基础功能，GLIBC 还提供了诸如线程等没有包含在 C 语言标准中的功能。GLIBC 提供的最主要功能都包含在静态库 `libc.a` 中，动态库则为 `libc.so`。
...
`crt1.o`、`crti.o`、`crtn.o` 三个目标文件包含了辅助程序运行的代码。`crt1.o` 中包含了真正的程序入口，也就是 `_start` 函数，位置在文件头所指定的 `Entry point address` 处。在本例中，可执行文件的入口地址是 `0x402d60`，反汇编可执行程序，在这个地址就是 `_start` 函数的反汇编内容。
...
其中地址 `0x402d95` 正是 `main` 函数的位置。随后，代码调用 `__libc_start_main` 开始初始化运行库，具体执行过程可以在 [GLIBC](https://sourceware.org/git/?p=glibc.git) 的源码中找到。函数定义在 `glibc/csu/libc-start.c` 中。
...
标准 C 库的入口函数是 `_start`，这也是默认链接脚本中指定的函数。在可执行文件的文件头中，有一项 `Entry point address`，其对应的入口地址就是 `_start` 函数的地址。代码中的 `main` 函数在标准库完成初始化后才由 `_start` 函数调用执行。
