...
屏幕前的你是否也和我一样，曾对 C 语言的一些功能感到困惑。例如，为什么在条件判断时会有“短路”求值的规则？为什么调用 `printf` 就能在屏幕上输出文本？或者你和我一样在学习这门语言时，对这些语言的规则深信不疑，把语言的规则当作是数学中给出的定义、定理一样，必须严格执行。数学运算必须在一个人给定的理论框架中进行，计算机语言的语法也是这样，但其中仍有区别。如果计算机语言的语法只能在纸面上执行，那么这些语法规则只是一些人为确定的抽象概念，与数学定理无异。不同的是，计算机语言的语法规范是要被**实现**的。实现一门语言意味着它的抽象概念能够被机器所执行，能够得到确定的结果。

计算机的实现依靠的是实现不同功能的电路，最基础的电路虽然只能够完成计算，但运用不同的物理定律，人类能够让电路实现特定的逻辑（逻辑门和其组合）。在这个逻辑之下，当电路接收到特定输入时，就能够有针对性的反应，得到特定结果作为输出。完成特定功能的电路可以被抽象成一个功能模块，让计算可以被控制。例如，如果让计算机执行加法运算，就要有一部分输入告诉计算机这是**加法**指令，另一部分提供加法的两个**加数**。控制执行哪些运算的部分就是控制器，具体执行计算的就是运算器。

这个直接操作计算机的规范就是机器语言，机器语言映射到的人类可读助记就是汇编语言。但是汇编语言只是机器语言的助记，只是减少了直接输入二进制数带来的繁琐步骤和混乱。而高级语言就是将一些常用的操作抽象，形成简单的函数，使用时只需要调用这个函数，不需要关心这个功能如何用机器语言来实现。例如内存申请功能，需要选择一块合适的内存区域使用，这需要考虑如何管理内存空间。在一个没有操作系统的设备上和在 Linux、Windows 等操作系统上，具体的实现方式一定不会是一样的。但对 C 语言的使用者来说，申请内存只需要调用 `malloc` 函数即可，在任何操作系统上都是这个函数。如果大家都用 C 语言编写程序，那么同一份代码就能在不同设备、系统上编译出可执行程序，在不考虑使用平台特定功能的情况下，程序的执行结果将会是相同的。这个大家都遵守的规范就是 C 语言的语法规范。不过，语言只确定了语法的规范，规定了什么样的语句应当有什么样的表现，但没有规定也不需要去规定这些规范在各种平台上要如何实现。在各个平台上实现语言规范，知道如何操作机器来满足规范的代码集合就是语言的运行时库（Runtime Library, RT），C 语言的运行时库就是 CRT。
